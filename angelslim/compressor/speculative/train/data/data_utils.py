...
def _image_row_segments(mask, factor, how):
    ...
    the tile structure and the separators exactly where they were. Matches
    tools/draft_bench.py:reduce_image_rows so that a checkpoint trained here
...
def reduce_vlm_image_rows(item, image_token_id, factor, how="pool", keep_positions=True):
    ...
    Reduces `hidden_states` (the 3-stream aux the draft consumes),
...
class VLMDataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Short-image mode: carry the second target forward's inputs through
        # per sample rather than batched. Its pixel_values have a per-image tile
        # count, and padding tiles would hand the vision tower embeddings that no
        # <image> token claims, so the trainer runs it one sample at a time.
        if any("short_input_ids" in item for item in features):
            batch["short_image"] = [
                {k: item[k] for k in ("short_input_ids", "short_pixel_values",
                                      "short_position_ids", "short_img_start",
                                      "short_span", "short_span_full")}
                if "short_input_ids" in item else {}
                for item in features
            ]
