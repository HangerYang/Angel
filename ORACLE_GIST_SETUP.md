...
The vLLM changes cannot be stored as traditional `.patch` files because they are complex edits across multiple methods. Instead, you have two options:

### Option A: Manual (detailed instructions)

Read [third_party/patches/GIST_PATCH_NOTES.md](third_party/patches/GIST_PATCH_NOTES.md) (created on this branch) for the exact edits needed to:
1. Add `eagle_gist.py`
2. Extend `llama_eagle3.py` with gist support
3. Extend `speculator.py` with oracle arming

Each section names the file, the anchor line, and what to add/change.

### Option B: Automated (Python script)

A setup script is included — check if `third_party/apply_gist_patches.py` exists on this branch:

```bash
python third_party/apply_gist_patches.py
```

It re-applies the edits automatically after `sync_vllm_latest.sh`.
