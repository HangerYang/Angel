...
if compgen -G "${LOG_ROOT}/recompute.jsonl.*" >/dev/null; then
  PYTHONPATH="${ODYSSEY}:${PYTHONPATH:-}" python3 -m odyssey.rescore \
    --events "${LOG_ROOT}/recompute.jsonl.*" \
    --out "${RES_ROOT}/rescored.jsonl" \
    --max_events "${MAX_RESCORE:-2000}" \
    || echo "rescore FAILED"
else
...
echo
echo "=== analysis ==="
PYTHONPATH="${ODYSSEY}:${PYTHONPATH:-}" python3 -m odyssey.analyze \
  --events "${LOG_ROOT}/*.jsonl.*" \
  --results_root "${RES_ROOT}" \
  --rescored "${RES_ROOT}/rescored.jsonl" \
  --out "${RES_ROOT}/report.json"
