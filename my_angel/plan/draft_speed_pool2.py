...
class Pooler(torch.nn.Module):
    """Proposed architecture: shared-routing attention pooling.

    Low-rank key (routing only), identity V, no o_proj, no MLP. Output is a
    convex combination of the target's own aux hidden states, so it lands in
    fc's expected space with no scale calibration. Cost over S is only the
    skinny k_proj; each extra aux layer is one more `w @ x` matmul.
    """
    def __init__(self, n_q, dim, dk=64):
        super().__init__()
        self.n_q = n_q
        self.dk = dk
        self.k_proj = torch.nn.Linear(dim, dk, bias=False, dtype=DT)
        self.q = torch.nn.Parameter(torch.randn(1, n_q, dk, dtype=DT) * 0.02)

    def forward(self, img):                      # img: [B, S, dim]
        k = self.k_proj(img)                     # [B, S, dk]
        w = torch.softmax((self.q @ k.transpose(1, 2)) * (self.dk ** -0.5), -1)
        return w @ img                           # [B, N, dim]
...
def run_one(m, aux, ids, img_mask, mixer, qformer, drop_img, timed_stages):
    ...
    for r in range(ROUNDS):
        ...
        # Reset the TTT layer-cache each round: in real drafting it is bounded by
        # the draft depth K, not by total tokens generated. Letting it grow to
        # ROUNDS*K makes the python `for i in range(1, lck)` loop dominate and
        # masks any effect of the prefill KV length.
        cache = c
...
VARIANTS = [
    ...
    ("P  pool N=208 (3L)",                    3, 1, False, False, "pool208"),
    ("P  pool N=52  (3L)",                    3, 1, False, False, "pool52"),
    ("P  pool N=16  (3L)",                    3, 1, False, False, "pool16"),
    ("P  pool N=4   (3L)",                    3, 1, False, False, "pool4"),
    ("P  pool N=16  (9L -> 3 bands)",         3, 3, False, False, "mixpool16"),
]
QF = {"qf4": (208, True), "qf8": (104, True), "qf16": (52, True), "qf4nm": (208, False)}
POOL = {"pool208": 208, "pool52": 52, "pool16": 16, "pool4": 4, "mixpool16": 16}
...
def main():
    ...
    ap.add_argument("--outdir", default="/home/hyang/AngelSlim/my_angel/plan/pool2")
    ...
    with open(f"{a.outdir}/report.txt", "w") as fh:
        ...
        for label, ns, pb, fcn, no, mode in VARIANTS:
            ...
            mixer = Mixer(ns, pb, H).to(DEV, DT).eval() if (mode in ("mix", "mixpool16") and pb > 1) else None
            qf = None
            if mode in QF:
                nq, mlp = QF[mode]
                qf = QFormer(nq, ns * H, mlp=mlp).to(DEV, DT).eval()
            elif mode in POOL:
                qf = Pooler(POOL[mode], ns * pb * H).to(DEV, DT).eval()
