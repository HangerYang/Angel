...
class Model(nn.Module):
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor, input_embeds=None):
        ...
        if hasattr(self, "stable_kv") and self.stable_kv is not None:
            ...
            inputs_embeds, _ = self.embed_model(input_ids[:, kv_len:])
            out_hidden, past_key_values = self(hidden_states, 
                                               input_ids=input_ids[:, kv_len:],
                                               inputs_embeds=inputs_embeds,
                                               past_key_values=self.stable_kv, use_cache=True)
        else:
            out_hidden, past_key_values = self(hidden_states, input_ids=input_ids, inputs_embeds=inputs_embeds, use_cache=True)
        ...
        for i in range(depth):
            ...
            out_hidden, past_key_values = self(input_hidden, input_ids=input_ids, inputs_embeds=inputs_embeds, past_key_values=past_key_values,
...
if __name__ == "__main__":
    config = EConfig.from_pretrained("hivis/train/vicuna_7B_config.json")
    model = Model(config, load_emb=True, path="llava-hf/llava-v1.6-vicuna-7b-hf")
    print(model)
