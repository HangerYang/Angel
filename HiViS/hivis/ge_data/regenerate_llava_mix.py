...
def parse_args():
    ...
    parser.add_argument(
        "--input_jsonl",
        type=str,
        default="eval_data/llava_v1_5_mix665k/llava_v1_5_mix665k_long_context.jsonl",
    )
    parser.add_argument(
        "--output_jsonl",
        type=str,
    ...
    parser.add_argument("--base_dataset_path", type=str, default="eval_data/llava_v1_5_mix665k/images")
    ...
    parser.add_argument(
        ...
        help=(
            "One or more vLLM base URLs. When provided, requests are dynamically "
            "balanced across these endpoints. Defaults to localhost ports "
            "8000-8003."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4096,
        help="Maximum total number of prompt and completion tokens.",
    )
    parser.add_argument("--temperature", type=float, default=0.0)
    parser.add_argument("--timeout", type=int, default=300)
    parser.add_argument("--num_workers", type=int, default=32)
    parser.add_argument("--max_inflight", type=int, default=64)
    parser.add_argument("--shuffle_seed", type=int, default=42)
    args = parser.parse_args()
    model_name = model_directory_name(args.model_path or args.vllm_model_name)
    output_dir = Path("eval_data/llava_v1_5_mix665k") / model_name
    if args.output_jsonl is None:
        args.output_jsonl = str(output_dir / "llava_v1_5_mix665k_regenerated.jsonl")
    if args.failed_jsonl is None:
        args.failed_jsonl = str(output_dir / "llava_v1_5_mix665k_regenerated_failed.jsonl")
    return args
...
def build_messages(history_conversations, image):
    buffer = BytesIO()
...
def call_vllm_chat(messages, args, endpoint_pool):
    ...
    for _ in range(3):
        ...
        if context_match is not None:
            message_tokens = int(context_match.group(1))
            allowed_completion = max(1, args.max_model_len - message_tokens)
            if message_tokens < args.max_model_len and allowed_completion < max_tokens:
                print(
                    f"[adjust] endpoint={endpoint}, context overflow fixed: "
        ...
        raise RuntimeError(
            f"vLLM endpoint {endpoint} HTTP {resp.status_code}: {err_text}"
        )

    raise RuntimeError(
        "vLLM context is too long after max_tokens adjustment"
    )
...
def regenerate_record(record, args, endpoint_pool):
    ...
    if conversations is None or image_relpath is None:
        return record
