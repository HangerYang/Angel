...
class Model(nn.Module):
    def __init__(
        ...
        if forward_num < 1:
            raise ValueError(f"forward_num must be positive, got {forward_num}")
        ...
        self.residual = nn.Parameter(
            torch.empty(forward_num, config.hidden_size)
        )
    ...
    def forward(
        ...
        residual = self.residual[0].expand(batch_size, seq_length, hidden_size)
        hidden_states = self.fc(torch.cat((inputs_embeds, hidden_states, residual), dim=-1))

        if q_hidden_states is not None:
            forward_num, _, _, _ = q_hidden_states.shape
            if forward_num >= self.residual.shape[0]:
                raise ValueError(f"q_hidden_states requires {forward_num + 1} residuals, but the model has {self.residual.shape[0]}")

            selected = self.residual[1:1 + forward_num]  # [forward_num, hidden_size]

            residual = selected[:, None, None, :].expand(forward_num, batch_size, seq_length, hidden_size)

            q_hidden_states = self.fc(torch.cat((inputs_embeds[None, :, :, :].expand(forward_num, -1, -1, -1), q_hidden_states, residual), dim=-1))
