...
parser.add_argument(
    ...
    dest='text_data_dir',
    type=str,
    default='eval_data/generated/llava16_7b/sharegpt',
    help='Directory containing first-stage text training samples.',
)
parser.add_argument(
    '--multimodal-data-dir',
    type=str,
    default=None,
    help='Directory containing first-stage multimodal training samples. If omitted, only text-data-dir is used.',
)
...
#     import wandb
#     wandb.init(project="", config=train_config)
    

def list_files(text_data_dir, multimodal_data_dir):
    datapath = []
    dirs = [("text", text_data_dir)]
    if multimodal_data_dir is not None:
        dirs.append(("multimodal", multimodal_data_dir))
    ...
    def __getitem__(self, index):
        ...
        if use_qwen_position_ids:
            if "position_ids" not in data:
                raise KeyError(
                    f"Qwen training sample is missing 'position_ids': {self.data[index]}"
                )
...
for epoch in range(num_epochs + 1):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        with accelerator.accumulate(model):
            ...
            predict = model(data["hidden_states"], **model_inputs)
            with torch.no_grad():
